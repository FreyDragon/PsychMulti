...
class PlayState extends MusicBeatState
{
	...
	public function forceTime(times:Float = 0) {
			if (FlxG.sound.music.time >= 0) {
				FlxG.sound.music.time = times;
				Conductor.songPosition = times;
			}
	}
	function resyncVocals():Void
	{
		...
		if (Main.instance.serverstate == "server") {
			...
			if (FlxG.sound.music.time % 1000 >= 4 && FlxG.sound.music.time >= 0.1) {
				Main.instance.sendServerMessage(["sync", Conductor.songPosition]);
			}
		} else {
			Conductor.songPosition = FlxG.sound.music.time;
		}
	...
	function set_health(value:Float):Float // You can alter how icon animations work here
	{
		...
		iconP2.animation.curAnim.curFrame = (healthBar.percent > 80) ? 1 : 0; //If health is over 80%, change opponent icon to frame 1 (losing icon), otherwise, frame 0 (normal)
			...
			iconP1.animation.curAnim.curFrame = (healthBar.percent > 80) ? 1 : 0; //If health is over 80%, change opponent icon to frame 1 (losing icon), otherwise, frame 0 (normal)
		}
		return health;
	}
	...
	function doDeathCheck(?skipHealthCheck:Bool = false) {
		...
		{
			if (Main.instance.serverstate == "client") {
				Main.instance.sendClientMessage(["BRO HES DEAD", "hi"]);
			}
			if (Main.instance.serverstate == "server") {
				Main.instance.sendServerMessage(["BRO HES DEAD", "hi"]);
			}
